...
class Model(object):
    ...
    def __init__(self, parameters=None, models_path=None, model_path=None):
        ...
        if model_path is None:
            ...
            #if a model with same parameter has been trained before, a new folder with differemt suffix will be create
            suffix_dir = 1
            fixed_len = len(model_path)
            while os.path.isdir(model_path):
                model_path = model_path[:fixed_len] + "_" + str(suffix_dir)
                suffix_dir += 1
